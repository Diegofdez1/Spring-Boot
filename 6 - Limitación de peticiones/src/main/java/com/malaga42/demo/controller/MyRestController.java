    ...
    public ResponseEntity<ApiResponse<ApiMember>> getMembers(@RequestParam(defaultValue = "1") String page) {
        try {
            if (requestsLimitation.getBucket().tryConsume(1)) {
                Page<Member> pageMembers = memberService.getMembers(Integer.parseInt(page) - 1, paginationLimit);
    ...
    public ResponseEntity<ApiResponse<String>> createMember(@RequestParam(name = "name") String name) {
        try {
            if (requestsLimitation.getBucket().tryConsume(1)) {
                ...
                if (member == null) {
                    new ResponseEntity<>(new ApiResponse<>(701, "Ya existe un miembro con ese nombre"), HttpStatus.OK);
                }
            } else {
                return new ResponseEntity<>(new ApiResponse<>(702, null), HttpStatus.TOO_MANY_REQUESTS);
            }
            ...
            log.error(ex.toString());
            return new ResponseEntity<>(new ApiResponse<>(500, "Ha ocurrido un problema a la hora de crear el miembro con nombre " + name), HttpStatus.INTERNAL_SERVER_ERROR);
        }

        return new ResponseEntity<>(new ApiResponse<>(200, "El miembro con nombre '" + name + "' creado con éxito"), HttpStatus.OK);
    }
